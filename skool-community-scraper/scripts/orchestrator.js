...
const TOOLS_DIR = path.join(PROJECT_ROOT, 'scripts');
...
${chalk.yellow('Usage:')}
  node scripts/orchestrator.js [options] <url|slug> [<url|slug> ...]

${chalk.yellow('Examples:')}
  ${chalk.dim('# Full pipeline for one community')}
  npm run scrape -- https://www.skool.com/aiautomationsbyjack

  ${chalk.dim('# Full pipeline for multiple communities')}
  npm run scrape -- https://www.skool.com/community1 https://www.skool.com/community2

  ${chalk.dim('# Only scrape (no downloads)')}
  npm run scrape -- --scrape-only https://www.skool.com/aiautomationsbyjack

  ${chalk.dim('# Only download (assumes scraping is done, pass slug names)')}
  npm run scrape -- --download-only aiautomationsbyjack

  ${chalk.dim('# Skip videos that already exist locally')}
  npm run scrape -- --skip-existing https://www.skool.com/aiautomationsbyjack
...
async function main() {
  ...
  if (!options.downloadOnly) {
    ...
    if (!cookieStatus.exists) {
      ...
      console.log(chalk.dim('  Run: ') + chalk.white('node scripts/validate-cookies.js'));
      console.log('');
      process.exit(1);
    }
    ...
    if (!cookieStatus.valid) {
      ...
      console.log(chalk.dim('  Run: ') + chalk.white('node scripts/validate-cookies.js --validate'));
      console.log('');
      process.exit(1);
    }
